...
const LoginForm: React.FC<LoginFormProps> = ({ onToggleForm }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!success) {
        setError('Invalid username or password. Try "johndoe", "janesmith", or "mikejohnson"');
      }
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-sm mx-auto bg-white border border-gray-300 rounded-lg p-8">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600 text-sm">Sign in to see photos and videos from your friends.</p>
      </div>
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-md">
        ...
        <p className="text-xs text-gray-600">Use these usernames: johndoe, janesmith, mikejohnson (any password)</p>
      </div>
    </div>
  );
};
